...
EmgResult emg_update(EmgDetector *ed, uint16_t sample, uint32_t now_ms)
{
    ...
    // Active when signal spikes HIGH (flexing), based on threshold value
    uint8_t above = (sample >= EMG_THRESHOLD);

    switch (ed->state) {

    // ── IDLE ──────────────────────────────────────────────────
    // wait state until start of a clench
    case EMG_STATE_IDLE:
    default:
        if (above) {
    ...
    // allow short (miss_ticks) before giving up
    case EMG_STATE_CLENCHING:
        ed->window_ticks++;

        if (above) {
            ed->clench_ticks++;
            ed->miss_ticks = 0;     // reset miss_tick counter on any good sample
        } else {
            ed->miss_ticks++;
        }
