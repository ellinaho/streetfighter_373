...
//x0 y0 z0 are the resting accelerometer values = "not moving"
...
// Main function called when new imu reading
PunchResult punch_update(PunchDetector *pd, int16_t raw_x, int16_t raw_y, int16_t raw_z, uint32_t now_ms)
{
    ...
    case PUNCH_STATE_ACTIVE:
        ...
        if (window_expired || naturally_ended) {
            ...
            //to stop  follow through motion from triggering from another punch detection
