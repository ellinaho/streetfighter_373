...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
// 0b000 for P1_ARM and 0b010 for P2_ARM
...
int main(void)
{
  ...
  while (1)
  {
    ...
    // emg_update calls the emg class that has its own state machine
    ...
    if (vibration) {
		...
		is_vibrating = 1;								// Set vibrating flag
    ...
    if (is_vibrating && ((HAL_GetTick() - vibration_start_time) >= VIBRATION_DURATION_MS)) {
        ...
        is_vibrating = 0;										// Clear vibrating flag
    }

    // ---- IMU read ----
    if (HAL_I2C_Master_Transmit(&hi2c1, SAD_W_M, &IMU_buf[6], 1, 10) != HAL_OK ||
        HAL_I2C_Master_Receive (&hi2c1, SAD_R_M,  IMU_buf, 6, 10) != HAL_OK) {
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART1) {
        if (rx_index == 0) {
            if (rx_byte == START_BYTE) { rx_buf[0] = rx_byte; rx_index = 1; }
        } else if (rx_index == 1) {
            rx_buf[1] = rx_byte; rx_index = 2;
        } else if (rx_index == 2) {
            rx_buf[2] = rx_byte; rx_index = 0;
            if (((rx_buf[1] & 0x38) >> 3) == DEVICE_ADDR && (rx_buf[1] & 0x07) == MAINSTM_ADDR)
                packet_received = 1;
        }
        HAL_UART_Receive_IT(&huart1, &rx_byte, 1);
    }
