module game_engine(
    ...
    wire one_second_pulse = (clk_counter == 26'd49_999_999);
	 wire six_second_pulse = (timer_6 == 3'd7);
    ...
    always @(posedge clk) begin
	 ...
	 reg [2:0] timer_6; // 7 bits can hold up to 127
    //assign time_left = timer_6;
    always @(posedge clk) begin
        if (rst || current_state == STARTUP) begin
            timer_6 <= 7'd6; // Reset time to 6
    ...
    always @(posedge clk) begin
        if (rst || current_state == STARTUP) begin
            round_timer <= 7'd16; // Reset time to 99
        end else if (current_state == PLAY && six_second_pulse) begin
